...
# Go through each image again extract feature and take votes create histogram
# Train classifier on histogram and labels
# Test classifier on histogram and labels
# create sift-vectors
...
def find_SVM(im_features, train_labels, kernel):
    ...
    svm = SVC(kernel=kernel, C=0.1, gamma=0.1, class_weight=None)
...
def train_model(no_clusters, kernel, extractor, images, labels):
    ...
    return visual_words, scale, svm, histograms_
...
def test_model(
    ...
    predictions = [i for i in svm.predict(kernel_test)]
